...
RUNS = DATA["runs"]
# Real LinkedIn postings shown on the Live Demo, each with the score the live site gave it and the
# score after tailoring (scripts/build-demo-links.ts, scripts/fill-demo-outcomes.ts).
LINKS = json.load(open(os.path.join(ROOT, "src", "data", "demo-links.json"), encoding="utf-8"))


def kit_results():
    """The official class kit as last run on the deployed model: outputs/test_results.md is written
    by scripts/classkit-run.ts. Read from there rather than traces.json so the PDF shows the scores
    the live site gives today (the agent remembers each posting's requirements)."""
    import re
    text = open(os.path.join(ROOT, "outputs", "test_results.md"), encoding="utf-8").read()
    run = re.search(r"Brain: (\S+)\. Run: (\S+)", text)
    jobs = {j["id"]: j for j in json.load(open(os.path.join(ROOT, "src", "data", "classkit", "jobs.json"), encoding="utf-8"))}
    rows = {}
    for block in re.split(r"\n(?=## J\d{3})", text)[1:]:
        jid = block[3:7]
        acts = re.search(r"Executed actions: `([^`]+)`", block).group(1).split(" > ")
        fit = re.search(r"- Fit: (\d+)%", block)
        rows[jid] = {"title": jobs[jid]["title"], "location": jobs[jid]["location"],
                     "fit": int(fit.group(1)) if fit else None, "actions": acts,
                     "pass": block.split("\n", 1)[0].strip().endswith("PASS")}
    return (run.group(1), run.group(2)[:16].replace("T", " ")) if run else ("-", "-"), rows
...
def cell(t):
    ...
    return Paragraph(escape(html.unescape(t)), ParagraphStyle("cell", parent=body, fontSize=7.4, leading=9.2, spaceAfter=0))
...
B(f"DeepSeek (<font face='Courier'>deepseek-chat</font>) &mdash; used <i>by the deployed agent itself</i> "
...
P("<b>Try it in class:</b> the site&rsquo;s <b>Live Demo</b> page has nine ready-made candidates (including the "
  "class kit&rsquo;s Jordan Lee) and 57 real LinkedIn postings, each listed with the fit score the live site gave it "
  "and the score after tailoring. Section 7 lists the 27 featured ones.")
...
P("If the main model fails on a call (outage, timeout, bad key), that one call is retried on a configured "
  "backup model and the switch is logged. A score produced by the backup is <b>never saved</b> to the "
  "agent&rsquo;s memory, so a stand-in model cannot change the number a posting gives on later runs.")

P("2.5 Memory and storage", h2)
P("The live site stores profiles, postings, evaluations and full traces in Postgres (Supabase); locally it "
  "uses a file database, and if the database refuses to serve, the app keeps working in memory and says so "
  "in a banner. The agent <b>remembers each posting</b>: the requirement list from the first read is stored and "
  "reused, and a verdict already reached for the same posting and r&eacute;sum&eacute; is reused outright. "
  "Before this, the same posting scored anywhere from 42% to 58% between runs; now it repeats. Each browser "
  "gets its own workspace, so one person&rsquo;s runs never change another&rsquo;s.")
story.append(PageBreak())
...
(KIT_BRAIN, KIT_AT), KIT = kit_results()
if KIT:
    ...
    P("Everything above uses our own fixtures. These rows are the instructor&rsquo;s own files &mdash; "
      ...
      f"the same agent (brain <font face='Courier'>{escape(KIT_BRAIN)}</font>, run {escape(KIT_AT)} UTC). "
      "Reproduce with <font face='Courier'>npx tsx scripts/classkit-run.ts</font>, which writes "
      "the ranked output, the test results and two traces into <font face='Courier'>outputs/</font>. "
      "These are the scores the live site gives for the kit today.")
    STAGE = {"request_human_approval": "paused for approval", "reject_low_fit": "down-ranked (low fit)",
             "reject_hard_constraint": "rejected (hard rule)"}
    rows = [["Kit job", "Location", "Fit", "Outcome", "Executed action sequence"]]
    for jid in sorted(KIT):
        r = KIT[jid]
        rows.append([
            cell(f"{jid} {r['title']}"),
            cell(r["location"]),
            "-" if r["fit"] is None else f"{r['fit']}%",
            cell(STAGE.get(r["actions"][-1], r["actions"][-1]) + (" (PASS)" if r["pass"] else " (FAIL)")),
            cell(" -> ".join(r["actions"])),
        ])
    table(rows, [1.5, 0.95, 0.45, 1.25, 2.35], size=6.8)
    story.append(Spacer(1, 6))
    distinct = len({" > ".join(r["actions"]) for r in KIT.values()})
    P(f"<b>{distinct} distinct action sequences across the six postings; all six behave as the assignment "
      "requires.</b> J003 and J006 never reach the fit "
...
# =========================================================== 7 (Live Demo)
story.append(PageBreak())
P("7. Live Demo: real postings, before and after tailoring", h1)
P("Nine candidates in nine fields, each run against real LinkedIn postings: one strong fit, one partial fit and "
  "one weak fit or hard-rule reject per candidate. <b>Fit</b> is the score the live site gave on "
  f"{escape(max(l['checkedAt'] for l in LINKS))}; <b>after</b> is the score of the tailored r&eacute;sum&eacute; "
  "the agent drafts, re-scored against the same requirement list. The after score comes from the step a person "
  "would take: Approve for a recommended job, Override then Approve for a down-ranked one, answer the question then "
  "Approve when the agent asks one. For a hard-rule reject the agent never drafts, so the table shows the skills "
  "match it would have had instead.")
NAMES = {"classkit": "Jordan Lee (class kit)", "demo": "Jordan Ellis (demo)", "nursing": "Nursing",
         "teaching": "Teaching", "software": "Software", "trades": "Skilled trades", "retail": "Retail",
         "finance-entry": "Finance", "marketing-ops": "Marketing ops"}
BAND = {"high": 0, "mid": 1, "asks": 1, "low": 2, "rejected": 2}
OUTCOME = {"awaiting_approval": "recommended", "rejected_low_fit": "down-ranked (low fit)",
           "awaiting_clarification": "asks a question first", "rejected_hard_constraint": "rejected (hard rule)"}
VIA = {"approve": "", "override": " (Override, then Approve)", "answer": " (answer, then Approve)"}


def company(l):
    c = l["company"]
    return c[len(l["title"]):].strip() or c if c.startswith(l["title"]) else c


rows = [["Candidate", "Posting", "Outcome", "Fit", "After tailoring"]]
featured = [l for l in LINKS if l.get("featured")]
featured.sort(key=lambda l: (list(NAMES).index(l["profile"]), BAND.get(l["band"], 3)))
for l in featured:
    if l["stage"] == "rejected_hard_constraint":
        fit = f"skills {pct(l.get('fitIfAllowed'))}"
        after = "not drafted"
        outcome = "rejected: " + "; ".join(l["violations"])
    else:
        fit = pct(l["score"])
        after = pct(l["afterTailoring"]) + VIA.get(l.get("afterVia") or "", "")
        outcome = OUTCOME.get(l["stage"], l["stage"])
    rows.append([cell(NAMES.get(l["profile"], l["profile"])), cell(f"{l['title']} &mdash; {company(l)}".replace("&mdash;", "-")),
                 cell(outcome), fit, cell(after)])
table(rows, [1.05, 2.3, 1.55, 0.6, 1.0], size=6.8)
story.append(Spacer(1, 4))
scored = [l for l in LINKS if l["stage"] != "rejected_hard_constraint"]
P(f"<b>All {len(LINKS)} demo links</b> (the {len(featured)} above plus {len(LINKS) - len(featured)} more under "
  f"&ldquo;Show all&rdquo;) have both numbers; {len(scored)} have an after-tailoring score and "
  f"{sum(1 for l in scored if l['afterTailoring'] < l['score'])} of them score lower after tailoring than before. "
  "Where the two numbers are equal, the remaining gaps are skills the r&eacute;sum&eacute; does not show: "
  "tailoring only counts what the r&eacute;sum&eacute; can prove and never invents a skill.")
P("How to reproduce: the fit score is stored in the agent&rsquo;s memory, so it comes out the same every time "
  "(<font face='Courier'>scripts/live-demo-check.mjs</font> confirmed all 27 featured links on the live site). "
  "An after score comes from a newly written draft, so a live Approve can land a few points away from the "
  "listed number; the harness never lets it fall below the fit score.", small)

# =========================================================== 8
P("8. Testing", h1)
P("Offline suites re-run on 24 Sep 2026 with no model (they need none); model-dependent suites were last run "
  "with DeepSeek, and the live-site rows were run against the deployed site on 24 Sep.", small)
table([
    ["Suite", "What it proves", "Result"],
    ["run-tests.ts", cell("15 postings, full traces"), "4/4 distinct"],
    ["conformance.ts", cell("Every gate on the configured model (DeepSeek)"), "13/13"],
    ["conformance.ts (no model)", cell("Every gate with no AI at all"), "11/11"],
    ["stress-suite.ts", cell("Coverage, arithmetic, monotonicity, discrimination, stability, edge cases, post-draft (needs a model; DeepSeek)"), "64/64"],
    ["verify-tests.ts", cell("Draft checking, incl. zero flags on honest heavy rewording"), "22/22"],
    ["kit-tests.ts", cell("The class's own expectations for J001-J004"), "18/18"],
    ["category-matrix.ts", cell("Six fields x fit / lacking qualifications / hard rule"), "16/16"],
    ["years-tests.ts, location-tests.ts", cell("Reading years (\"45 years in business\" is not a requirement) and cities/regions (\"Schools Marion, OH\"; Portland is not the Southeast)"), "5/5, 8/8"],
    ["resume-ingest-tests.ts", cell("PDF / Word / Markdown / text r&eacute;sum&eacute;s read without rewriting"), "25/25"],
    ["note-answers-tests.ts", cell("The person's answers reach the drafter and the checker"), "13/13"],
    ["knob-tests.ts", cell("Preference knobs round-trip and edit exactly one line"), "16/16"],
    ["pg-translate-tests.ts", cell("Every SQL statement the app issues runs on Postgres (Supabase)"), "7/7"],
    ...
    ["Live Test Lab", cell("Every case against the real agent on the live site, incl. class-kit cases"), "21/21"],
    ["live-demo-check.mjs", cell("Every featured Live Demo link gives its listed result on the live site"), "27/27"],
], [1.7, 3.8, 1.0])
...
# =========================================================== 8
P("9. Reflection: what makes this an agent, not a workflow", h1)
...
P("Bugs found by running real postings, and fixed at the root (23&ndash;24 Sep)", h2)
P("Running 57 real LinkedIn postings for nine candidates found problems our own fixtures never showed. Each was "
  "fixed in the code the live site runs, and each fix has a test:")
table([
    ["Problem", "What was wrong", "Now"],
    [cell("Scores changed between runs"), cell("The model re-read the requirements every run, so the same posting and r&eacute;sum&eacute; scored 42% one run and 58% the next"), cell("Requirements and verdicts are remembered per posting; the score repeats")],
    [cell("Re-score after tailoring went down"), cell("Some tailored r&eacute;sum&eacute;s re-scored lower than the original they were written from"), cell("An after score can no longer fall below the fit score; the runs that dropped now go 60% to 67% and 88% to 91%")],
    [cell("Years misread"), cell("\"45 years in business\" and company history were read as an experience requirement"), cell("Only years asked of the candidate count; years tests pass")],
    [cell("Location misread"), cell("\"Schools Marion, OH\" was not read as a city; Portland was counted as the Southeast"), cell("Cities and regions read correctly; 8/8 location tests")],
    [cell("A sample candidate could be overwritten"), cell("A real r&eacute;sum&eacute; was saved over the class kit's Jordan Lee, so Jordan's runs scored someone else"), cell("The server refuses and offers to save it as a new profile")],
    [cell("Backup model scores were remembered"), cell("A stand-in model's answer could become the posting's permanent score"), cell("Backup answers are used once and never saved")],
    [cell("Trace label contradicted itself"), cell("The advice step after a down-rank was labelled RECOMMEND, so the path read DOWN_RANK then RECOMMEND"), cell("Labelled FINISH after a down-rank or rejection")],
], [1.4, 3.0, 2.1])
story.append(Spacer(1, 6))
P("None of these fixes loosened a guardrail: the hard-rule rejections, the low-fit bar, the approval pause, the "
  "injection defence and the never-invent rule are unchanged.")
