...
| `scan_for_injection` | Scans the posting for text aimed at an AI screener. **Two layers merged:** a built-in regex floor (`scanForInjection()`, 30 patterns, always runs) OR passages reported by the AI reader (`assessPosting()`), each verified as a literal substring of the posting. `injectionSources` records which layer caught it | Must happen before anything else touches the posting text, so nothing downstream can be steered by it |
