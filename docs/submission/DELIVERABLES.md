...
|---|---|---|---|
| 1 | Working prototype | The whole app | https://g6-job-search-agent-g6.vercel.app — paste a posting on *Add job*, or run the Test Lab |
...
| 6 | Ranked job output | `outputs/ranked_jobs.md`, and the dashboard ranking in the app | `npx tsx scripts/classkit-run.ts` |
...
| 8 | Four test results | `outputs/test_results.md` (the kit's J001–J006), and the Test Lab's 23 cases in the app | `npx tsx scripts/classkit-run.ts`; Test Lab → Run all |
...
| 10 | Two detailed traces | `outputs/trace_J001.json` (recommend) and `outputs/trace_J004.json` (injection), plus `docs/submission/traces.json` | Open the files, or the "Decision trace" panel on any job |
...
| Evaluation / testing | 15 | The kit's six cases, the Test Lab's 23, and eleven command-line suites (`TESTING-GUIDE.md`) |
